...
def get_capacity(grid_input_stre):
    bool_couple = 0
    bool_place = 0
    bool_sum = 0
    thres = 0.4
    map_num = 5
    while bool_sum < 2:
        a, c = get_score(int(map_num), float(grid_input_stre), float(20))
...
cap_coupled = jnp.zeros(num_grid_input)
cap_place = jnp.zeros(num_grid_input)
score_diff = jnp.zeros(num_grid_input)
for j in range(num_grid_input):
   bst.random.seed(j)
   a, b, c = get_capacity(grid_input_stre=grid_input[j])
   cap_coupled = cap_coupled.at[j].set(a)
   cap_place = cap_place.at[j].set(b)
   score_diff = cap_place.at[j].set(c)
   print('Progress:', (j+1)/num_grid_input*100,'%')
