...
from utils.helperFunctions import generate_stim_array,circ_dis
...
def run(args):
    ...
    pos_array = generate_stim_array(L = args.num_simulation, k = 20, a = 98.5)
    ...
    for i in range(args.num_simulation):
        ...
        pos_item = float(pos_array[i])
        pos_gt = torch.tensor(pos_item, dtype=torch.float32)  # ground truth position
        ...
        z_t = pos_gt - 0.9 * params_prob["v"] * params_prob["dt"] # previous position
        ...
        decoding_dict['pos_GOP'] = z_est_GOP
        decoding_dict['pos_NET'] = z_est_net
        for module in range(params_PSC['M']):
            decoding_dict[f'phi{module+1}_GOP'] = phi_est_GOP[module]
            decoding_dict[f'phi{module+1}_NET'] = phi_est_net[module]
        # Save results
...
if __name__ == "__main__":    
    ...
    arg_parser.add_argument('--num_simulation', '-n', type=int, default=400, help="simulation number")
