...
def parallel_trial(i):
    
    z_truth = 62
    z_m = z_truth-0.1
    phi_m = np.mod(z_m / Lambda, 1) * 2 * np.pi 
    z_e = z_truth+0.1
    psi = phi_m + sigma_phi * np.random.randn(M) * ratio
    psi = phi_m
    Ig = np.zeros((M, num_g))
    for j in range(M):
    ...
    return z_decode_e, z_decode_m, z_decode_b
...
if __name__ == "__main__":
    ...
    with Pool() as pool:
        results = pool.map(parallel_trial, range(trial_num))
    
    for i, (z_decode_e, z_decode_m, z_decode_b) in enumerate(results):
