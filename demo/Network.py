...
def Net_decoding():
    ...
    # cell number
    num_p = 1280
    num_g = int(num_p/L*2*np.pi) # 为了让两个网络的rho相等
    M = len(Lambda)
    # connection range
    a_p = 4
...
class Place_net(bp.DynamicalSystem):
    ...
    def get_center(self):
        ...
        exppos = bm.exp(1j * x)
        self.center[0] = (bm.angle(bm.sum(exppos * self.r))/2/np.pi + 1/2) * self.z_range
...
class Grid_net(bp.DynamicalSystem):
    def __init__(self, L, z_min, z_max, num=100, num_hpc=1000, k_mec=1.,
        ...
        # feature space
        self.x_hpc = np.linspace(z_min, z_max, num_hpc, endpoint=False)  # The encoded feature values
        self.x = np.linspace(0,2* np.pi, num, endpoint=False)  # The encoded feature values
        self.rho = num / np.pi/2  # The neural density
        self.dx = np.pi*2  / num  # The stimulus density
    ...
    def xtopi(self,x):
        return (x%self.L)/self.L*2*np.pi
    ...
    def reset_state(self):
        ...
        self.v.value = bm.Variable(bm.zeros(self.num))
        self.input.value = bm.Variable(bm.zeros(self.num))
        self.center.value = 2 * bm.pi * bm.random.rand(1) 

    def get_center(self):
        X = self.x - bm.pi
        exppos = bm.exp(1j * X)
        self.center[0] = bm.angle(bm.sum(exppos * self.r)) + bm.pi
