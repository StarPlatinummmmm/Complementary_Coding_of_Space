...
class LSC(nn.Module):
    ...
    def forward(self, x, noiseFlag=False):
        ...
        if noiseFlag:
            fr_noise = torch.normal(0, self.sigma_p, size=(self.n_p,)).to(device)
        else:
            fr_noise = 0.0
        activation_p += fr_noise
    ...
    def forward_prime(self, x):# phi is a vector
        ...
        r_p_prime = 1.*torch.exp(-0.5 * distance**2 / (math.sqrt(2)*self.a_p)**2) * distance/ (math.sqrt(2) * self.a_p)**2
        return r_p_prime 
...
class PSC(nn.Module):
    ...
    def forward(self, x, noiseFlag=False, pi_ampl=1.0):
