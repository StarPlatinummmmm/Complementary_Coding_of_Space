...
class Place_net(bst.Module):
  def __init__(self, z_min, z_max, map_num=2, neuron_num=1280, place_num=128, k=1.,
               tau=10., a_p=0.5, J0=5., noise_stre=0.05, randomkey=0):
    super().__init__()
  ...
  def generate_maps(self, ):
    ...
    key = jax.random.PRNGKey(self.randomkey)
    for i in range(self.map_num):
      key, subkey = jax.random.split(key)
      map.at[i].set(jax.random.permutation(subkey,x)) 
      sampled_integers = random.sample(range(self.neuron_num), self.place_num)
      place_index.at[i].set(jnp.array(sampled_integers, dtype=int))
      place_index.at[i].set(jnp.sort(place_index[i]))
  ...
  def get_center(self, map_index):
    ...
    exppos = jnp.exp(1j * x)
    self.center[0] = (jnp.angle(jnp.sum(exppos * r)) / 2 / jnp.pi + 1 / 2) * self.z_range + self.z_min
  ...
  def update(self, loc=0., map_index=0, input_stre=1, input_g=0.):
    ...
    Irec = jnp.matmul(self.conn_mat, self.r)
    ...
    noise = self.noise_stre * jax.random.normal(subkey, shape=(self.neuron_num,))

    du = (-self.u + self.input + Irec + noise + input_g) / self.tau * jnp.dt
    u = self.u + du
    self.u.value = jnp.where(u > 0, u, 0)
    r1 = jnp.square(self.u)
...
class Grid_net(bst.Module):
  def __init__(self, L, maps, place_index, neuron_num=100, k_mec=1.,
               tau=1., a_g=1., J0=50., W0=0.1, randomkey=0):
    super().__init__()
    ...
    # 创建PRNGKey
    key = jax.random.PRNGKey(randomkey)

    self.phase = jax.random.uniform(key, shape=(self.map_num,)) * self.L 

    # self.phase = jnp.random.rand(self.map_num) * self.L 
    # Connections
    ...
    for map_index in range(self.map_num):
      ...
      conn_out = jax.ops.index_add(conn_out, jax.ops.index[self.place_index[map_index], :], out)
  ...
  def get_center(self):
    ...
    self.center[0] = jnp.angle(jnp.sum(exppos * self.r))
    self.center_input[0] = jnp.angle(jnp.sum(exppos * self.input))
  ...
  def update(self, r_hpc, loc=10, input_stre=1., map_index=0):
    ...
    # Calculate recurrent input
    r_fft = jnp.fft.fft(self.r)
    Irec = jnp.real(jnp.fft.ifft(r_fft * self.conn_fft))  # real and abs
    # Calculate total input
    self.input.value = input_hpc + Ig
    # Update neural state
    du = (-self.u + self.input + Irec + I_per) / self.tau * jnp.dt
    u = self.u + du
    self.u.value = jnp.where(u > 0, u, 0)
    r1 = jnp.square(self.u)
    r2 = 1.0 + self.k * jnp.sum(r1)
    self.r.value = r1 / r2
    self.output = jnp.matmul(self.conn_out, self.r)
