...
def run(args):
    ...
    decoding_dict = dict() # keys: pos (scalar), phi (vector)
    ...
    for i in range(args.num_simulation):
        ...
        activation_gs = PSCModel.forward(pos_gt, noiseFlag=True, pi_ampl=5.0)
        ...
        decoding_dict['pos_MAP'] = z_est_MAP
        for module in range(params_PSC['M']):
            decoding_dict[f'phi{module+1}_MAP'] = phi_est_MAP[module]
