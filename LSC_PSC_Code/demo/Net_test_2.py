...
sigma_g = np.sqrt(np.sqrt(np.pi)*Ag**3*rho_g*tau_g/(a_g*alpha_g)) * sigma_ratio
sigma_phi = 1/((Lambda/2/np.pi) * np.sqrt(J_pg*rho_g*Rg/(4*Ap*tau_p))) * sigma_ratio
sigma_p = np.sqrt(np.sqrt(np.pi)*Ap**3*rho_p*tau_p/(a_p*alpha_p)) * sigma_ratio
...
def Net_decoding(Ip, Ig):
    ...
    def run_net(i, Ip, Ig): 
        ...
        log_posterior = calculate_posterior(z_decode, phi_decode, Ig, Ip)
        return u_HPC, u_grid_1, u_grid_2, u_grid_3, I_mec, z_decode, phi_decode, log_posterior

    T = 5000
    indices = np.arange(T)
    z0 = z_truth - 1.
    # phi_0 = phi_truth - 0.05 * np.randn(M)
    ...
    if animate == 1:
        ...
        ax_ani[0][1].set_xlim(0, 2*np.pi)
        ax_ani[0][1].set_ylim(-1, y_lim)
        ax_ani[1][0].set_xlim(0, 2*np.pi)
        ax_ani[1][0].set_ylim(-1, y_lim)
        ax_ani[1][1].set_xlim(0, 2*np.pi)
