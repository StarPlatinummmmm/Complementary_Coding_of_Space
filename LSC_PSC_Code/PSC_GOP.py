...
def PSC_GOP_decoder(PSCModel,total_iterations, z_t, phi_t, activation_gs):  
    ...
    for iteration in range(total_iterations):
        ...
        # Ig_fgprime_prod = activation_gs * fg_prime # shape [n_g*M]
        # Ig_fgprime_prod = Ig_fgprime_prod.reshape(params_PSC['n_g'],params_PSC['M'])
        # Ig_fgprime_prod = torch.sum(Ig_fgprime_prod, dim=0) # shape [M]
...
def run(args):
    ...
    pos_gt = torch.tensor([62.], dtype=torch.float32)  # ground truth position
