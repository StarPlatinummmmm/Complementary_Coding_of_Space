...
for i in range(M):
    ...
    Ig[i, :] = np.exp(-dis_theta**2 / (4 * a_g[i] ** 2)) + sigma_g[i] * ratio * np.random.randn(num_g)
x = np.linspace(0,L,num_p,endpoint=False)
dis_x = x-z_truth
...
def calculate_posterior(z, phi, Ig, Ip):
    x = np.linspace(0,L,num_p,endpoint=False)
    theta = np.linspace(0,2*np.pi,num_g,endpoint=False)
    psi_z = bm.mod(z / Lambda, 1) * 2 * np.pi
    log_prior = 0
    log_likelihood_grid = 0
    for i in range(M):
        dis_1 = circ_dis(theta, phi[i])
        fg = bm.exp(-dis_1**2 / (4 * a_g[i]**2))
        log_likelihood_grid -= bm.sum((Ig[i, :] - fg)**2) / sigma_g[i]**2
        dis_2 = circ_dis(phi[i], psi_z[i])
        log_prior -= 1 / (sigma_phi[i]**2) * bm.exp(-dis_2**2/8/a_g[i]**2) * dis_2**2
    dis_x = x - z
...
def Net_decoding(Ip, Ig, alpha_p = 0.05, alpha_g = 0.05, plot=1, animate =0):
    ### Place cells
    ...
    def run_net(i, Ip, Ig): 
        ...
        log_posterior = calculate_posterior(z_decode, phi_decode+bm.pi, Ig, Ip)
        return u_HPC, u_grid_1, u_grid_2, u_grid_3, I_mec, z_decode, phi_decode, log_posterior
    ...
    u_HPC, u_grid_1, u_grid_2, u_grid_3, I_mec, z_record, phi_record, log_posterior = bm.for_loop(run_net, (indices, I_place, I_grid), progress_bar=True)
    ...
    if plot == 1:
        ...
        plt.plot(x_hpc-62, Ip, label='Place input', linewidth=2., color = (78/255, 183/255, 233/255))
        plt.plot(x_hpc-62, u_HPC[-1], label='Stationary state', linewidth=linewidth, color = (220/255, 64/255, 26/255))
        plt.xlim(-10, 10)
        plt.ylim(-0.5, y_lim)
        plt.xlabel('$x$', fontsize=fontsize)
        # plt.ylabel('Neural activities', fontsize=fontsize)
        ...
        plt.tight_layout()
        plt.figure(figsize=(6, 3))
        plt.plot(theta-np.pi, Ig[0], label='Grid input', linewidth=2., color = (78/255, 183/255, 233/255))
        plt.plot(theta-np.pi, u_grid_1[-1,:], label='Stationary state', linewidth=linewidth, color = (220/255, 64/255, 26/255))
        ...
        plt.figure()
        plt.plot(-log_posterior, 'b', linewidth=linewidth)
        plt.xlabel('T', fontsize=12)
        plt.ylabel('-ln p(z,φ|rg, rp)', fontsize=fontsize)
        plt.gca().tick_params(width=linewidth)
        plt.gca().tick_params(labelsize=fontsize)
        plt.tight_layout()
    ...
    if animate == 1:
        ...
        ax_ani[0][0].legend(['I_p','u_p', 'W_gp r_g'])
        ax_ani[0][1].legend(['I_g1', 'u_g1'])
        ax_ani[1][0].legend(['I_g2', 'u_g2'])
        ax_ani[1][1].legend(['I_g3', 'u_g3'])
        # 更新线条的函数
        def update(frame):
        ...
        ani = FuncAnimation(fig, update, frames=T, interval=20, blit=True)# 保存动画
