...
def run(args):
    ...
    phi_ampls = np.array([0.5, 1.0, 1.5, 2.0, 2.4, 2.8, 3.4, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0])
    
    n_phi_ampls = len(phi_ampls)
    n_methods = 3
    z_est_arrays = np.zeros([n_phi_ampls, args.num_simulation, n_methods])
    # print('z_est_arrays shape:', z_est_arrays.shape)
    phi_est_arrays = np.zeros([n_phi_ampls, args.num_simulation, n_methods, params_PSC['M']])
    
    for pi_index, phi_ampl in enumerate(phi_ampls):
        # print('pi_index:', pi_index, 'phi_ampl:', phi_ampl)
        phi_ampl = float(phi_ampl)
        for n_index in range(args.num_simulation):
            ...
            activation_gs = PSCModel.forward(pos_gt, noiseFlag=True, phi_ampl=phi_ampl)
